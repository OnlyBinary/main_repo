...
<%--                        <p>${s.cnt}</p>--%>
        ...
        <div class="col-lg-6">
            <div class="card shadow mb-4" style="height:50vh;">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">오늘 종료 행사</h6>
                </div>
                <div class="card-body" style="overflow:scroll;">
                    <c:forEach var="t" items="${todayFinished}" varStatus="status">
                        <div class="p-3 mb-2 text-bg-light rounded-3">
                            <p>${status.index+1}. ${t.svcnm}</p>
<%--                            <p>serviceId: ${r.svcid}</p>--%>
                            <p>${t.svcstr.toString().split("T")[0]} ~ ${t.svcfin.toString().split("T")[0]}</p>
                        </div>
                    </c:forEach>
                </div>
            </div>
